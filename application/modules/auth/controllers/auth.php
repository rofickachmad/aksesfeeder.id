...
class Auth extends MX_Controller {
    ...
    function __construct()
    {
        ...
        $this->session->set_userdata("module",'auth');
    }
        ...
        if ($this->form_validation->run() == true)
        {
            ...
            if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember))
            {
                ...
                if($this->ion_auth->in_group(1,2)){ //administrator/pimpinan
                    ...
                    }elseif($this->ion_auth->in_group(12)){//mahasiswa/calon/pendaftar
                        redirect(base_url('pmb/frontline'), 'refresh');
    ...
    //edit a edit profile mahasiswa dan dosen
    function editmhs($id=null){
        if ( !$this->ion_auth->logged_in()): 
            redirect('auth/login', 'refresh');
            echo pesan_login('mhs');
        else:
       ...
       /* if (!$this->ion_auth->logged_in() || (!$this->ion_auth->is_admin() && !($this->ion_auth->user()->row()->id == $id)))
        ...
        // $this->template->set_layout('dashboard');
            $this->data['view']='auth/pmb/edit_user';
            $this->data['title']='Edit User';
            // $html=$this->load->view('wrapper',$this->data,false);
            // $this->output->set_output($html);

        $this->_render_page('auth/mhs/editmhs', $this->data);
    }
        //edit user
