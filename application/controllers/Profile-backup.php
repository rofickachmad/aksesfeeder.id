...
class Profile extends MX_Controller {
	...
	public function index(){
		...
		// $pass=$this->ws->gettoken();
		/*$token =array(
			'act' => 'GetToken',
			'username' => $this->ws->username,
			'password' => $this->ws->password
		);
		$_token=json_decode($this->ws->execute($token));
		print_r($_token->data->token);*/
		$mhs = array(
		...
		print_r($this->ws->execute($mhs));
	}
	public function webservicesoap(){
		$url="http://192.168.56.101:8082/ws/sandbox.php?wsdl";
		$soap=$this->ws->getsoap($url);
		$connect=$this->ws->getconnect($url);
		$error=$connect['error'];
		$proxy=$connect['proxy'];
